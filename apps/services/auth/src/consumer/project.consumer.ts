...
const operation : Record<keyof typeof $ProjectEvent,(projectId:string,userId:string)=>Promise<void>> = {
                    CREATED:async(projectId,userId)=>{
                       try {
                         ...
                         logger.error(`ERR_PROCESSING_CREATED_EVENT_FOR_PROJECT_ID_${projectId}_USER_ID_${userId}_IN_AUTH_SERVICE: ${JSON.stringify(e, null, 2)}`);
                       }
                    },
                    DELETED:async(projectId,userId)=>{
                        try {
                            ...
                            logger.error(`ERR_PROCESSING_DELETED_EVENT_FOR_PROJECT_ID_${projectId}_USER_ID_${userId}_IN_AUTH_SERVICE: ${JSON.stringify(e, null, 2)}`);  
                        }
                    }
}
...
export async function startProjectConsumer(){
        ...
        await projectConsumer.readUnackedMessages(async({projectId,userId,event})=>{
