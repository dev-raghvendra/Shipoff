...
COPY apps/services/orchestrator ./apps/services/orchestrator

# Install all dependencies (dev + prod) needed for building TS
RUN pnpm install -r \
    --filter . \
    --filter ./apps/services/orchestrator \
...
COPY --from=builder /app/apps/services/orchestrator/dist ./apps/services/orchestrator/dist
COPY --from=builder /app/apps/services/orchestrator/package.json ./apps/services/orchestrator/package.json
...
COPY --from=builder /app/apps/services/orchestrator/node_modules ./apps/services/orchestrator/node_modules
