...
COPY --from=builder /app/apps/services/logs/dist ./apps/services/logs/dist
COPY --from=builder /app/apps/services/logs/package.json ./apps/services/logs/package.json
...
COPY --from=builder /app/apps/services/logs/node_modules ./apps/services/logs/node_modules

# Prune dev dependencies to shrink image
RUN pnpm prune --prod

# Set working directory for the service
WORKDIR /app/apps/services/logs
